...
export default function ProductsPage() {
  ...
  return <div><PageHeader title="Products" description="Search, filter, and maintain the complete product catalog." action={<button className="btn-primary" onClick={() => setOpen(true)}><Plus className="size-4"/>Add product</button>} />
    <div className="card mb-5"><label className="relative block max-w-md"><Search className="absolute left-3 top-2.5 size-4 text-slate-400"/><input className="field pl-9" placeholder="Search by product name or SKU" value={search} onChange={(e) => setSearch(e.target.value)} /></label></div>
